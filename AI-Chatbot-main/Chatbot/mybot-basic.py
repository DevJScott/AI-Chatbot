...
# Check KB integrity after loading
def check_kb_integrity():
    for i, expr1 in enumerate(kb):
        for j, expr2 in enumerate(kb):
            if i != j:
                if ResolutionProver().prove(expr1.negate(), [expr2]):
                    print(f"Warning: Contradiction found between {expr1} and {expr2}")
                    return False
    return True


# Call this after loading the KB
if kb and check_kb_integrity():
    print("KB integrity check passed - no contradictions found.")
else:
    print("Warning: KB may contain contradictions!")
